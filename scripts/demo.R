...
download.file('https://zenodo.org/records/8308733/files/rossyndicate/CPF_reservoir_study_data-latest.zip?download=1',destfile = 'ross_cpf.zip') 
# unzip this file
unzip('ross_cpf.zip') 
...
no3_example_plot <- most_recent_chem %>% 
  filter(site_code %in% c("JOEI","CBRI", "CHD", "JWC" )& Year > 2020)%>%
